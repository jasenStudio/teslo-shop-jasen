...
export const placeOrder = async (
  ...
  if (!userId) {
    return {
      ok: false,
      message: "No hay session de usuariop",
    };
  }

  //Obtener la informaci{on de los productos
  ...
  //* Crear la transacion de la base de datos;
  ...
  try {
    ...
    const prismaTx = await prisma.$transaction(async (tx) => {
        ...
        //2. Crear la orden header = el detalle
